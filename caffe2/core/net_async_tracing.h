...
int CAFFE2_API extractShardId(const std::string& name);

// Check if the net name is white-listed for tracing (specified via a command
// line flag)
bool CAFFE2_API isTraceableNetName(const std::string& net_name);

std::shared_ptr<Tracer> CAFFE2_API create(const NetBase* net, const std::string& net_name);
bool CAFFE2_API startIter(const std::shared_ptr<Tracer>& tracer);
